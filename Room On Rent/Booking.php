...
if(!isset($_SESSION['user_id']))
{
	...
	header("Location:Login.php?page=Booking.php&room=$P");
}
else
{
	$CHem=$_SESSION['user_id'];
	$P=$_GET['room'];
	mysql_connect("localhost","root","");
	mysql_select_db("room");
	$query = "SELECT * FROM room_details WHERE roomno='$P'";
	...
	if($CHem==$arr[0])
			header("Location:Book.php?err=1&room=$Room");			 
echo "	
<br><br>
  	...
  	echo "<img src='$arr[5]' style='height:200px;width:200px;'</img><br><br><br>";
		echo "
			<form action='Booked.php' method='post'>
  			<input type='hidden' name='roomno' value='$P'>
  			<input class='ui button blue' type='submit' name='submit' value='Book this room'>
				</form>    
  	";
  	echo "</center><br><br>";
  	echo "</div>";
